report 50205 "AICASA EMT-Basic & Advanced"
{
    Caption = 'AICASA EMT-Basic & Advanced Certificate';
    DefaultLayout = RDLC;
    RDLCLayout = './src/reportrdlc/AICASA EMT-Basic & Advanced Certificate.rdl';
    ...
    dataset
    {
        dataitem("Student Degree"; "Student Degree")
        {
            ...
            { }
            column(Certificate_Date; "Certificate Date")
            { }
            column(Expiration_Date; "Expiration Date")
            { }
            column(Degree_Code; "Degree Code")
            { }
            column(BasicDegree; BasicDegree)
            { }
            column(AdvanceDegree; AdvanceDegree)
            { }
            ...
            begin
                ...
                IF StudentDegree.FindFirst() then begin
                    IF NOT (StudentDegree."Degree Code" In ['EMTAL', 'EMTALREC', 'EMTBL', 'EMTFR', 'EMTPA', 'EMTPD']) then
                ...
                if "Certificate Date" = 0D then
                    Error('Certificate date not found')
                else
                    DateAwarded1 := "Certificate Date";

                BasicDegree := false;
                IF "Student Degree"."Degree Code" In ['EMTBL', 'EMTFR', 'EMTPA', 'EMTPD'] then
                    BasicDegree := true;

                AdvanceDegree := false;
                IF "Student Degree"."Degree Code" In ['EMTAL', 'EMTALREC'] then
                    AdvanceDegree := true;
