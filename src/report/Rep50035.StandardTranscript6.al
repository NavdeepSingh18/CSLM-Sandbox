report 50035 StandardTranscript6
{
    Caption = 'Standard Transcript';
    DefaultLayout = RDLC;
    RDLCLayout = './src/reportrdlc/StandardTranscript6.rdl';
    ...
    dataset
    {
                            ...
                            IF (AcademicYear_gTxt <> MainStudentSubject."Academic Year") or (Term_gTxt <> MainStudentSubject."Term Sequence") then begin
                                IF (MainStudentSubject.Course IN ['AUA-GHT', 'FIU - AUA CLINICAL', 'FIUGLOBAL']) then begin
                                    ...
                                    IF AUAGHTBanner <> 'AUA-GHT|FIU - AUA CLINICAL|FIUGLOBAL' then begin
                                        TempRecord1.Init();
                                        EntryNo := EntryNo + 1;
                                        TempRecord1."Competition type" := Format(EntryNo);
                                ...
                                IF MainStudentSubject_gRec1.FindSet() then begin
                                    repeat
                                        ...
                                        MainStudentSubject_lRec.SetRange("Term Sequence", MainStudentSubject."Term Sequence");
                                    ...
                                    IF SqNo = 1 then
                                        If DeanHonorRoll then begin
                                            If (CreditConsider <> 0) and (GrandGPA <> 0) then
                                                TempRecord1.Field51 := 'Term GPA :' + ' ' + Format(Round((TotalPoint / CreditConsider)), 0, '<Precision,2:2><Sign><Integer Thousand><Decimals>') + '     ' + 'CUM GPA :' + ' ' + Format(Round(((GrandPoint + GrandPoint1) / GrandGPA)), 0, '<Precision,2:2><Sign><Integer Thousand><Decimals>') + '     ' + TempRecord1.FieldCaption("Dean's Honor Roll")
                                            Else
                                                if (CreditConsider = 0) and (GrandGPA <> 0) then
        ...
        dataitem(TempRecord; "Setup Co-Curricular Points -CS")
        {
    ...
    var
        ...
        TempRecord1: Record "Setup Co-Curricular Points -CS";
        TempRecordTemp: Record "Setup Co-Curricular Points -CS";
