/*
codeunit 50052 StudentLayoutStatementCs
{
    ...
    var
        StatementFileNameTxt: Label 'Student Statement', Comment = 'Shortened form of ''Student Statement''';
        StatementReportNotFoundErr: Label 'No Student statement report has been set up.';
        RunCustomerStatementsTxt: Label 'Run Student Statements from Job Queue.';
        DuplicateJobQueueRecordErr: Label 'Student statements are already scheduled to run in the job queue.';
        ConfirmRunRepInBackgroundQst: Label 'Do you want to set the job queue entry up to run immediately?';

    [Scope('OnPrem')]
    procedure RunReport()
    var
        Customer: Record "Student Master-CS";
        ReportSelections: Record "Report Selections";
        CustomLayoutReporting: Codeunit CustomLayoutReportingcs;
        RecRef: RecordRef;
    begin
        RecRef.Open(DATABASE::"Student Master-CS");
        CustomLayoutReporting.SetOutputFileBaseName(StatementFileNameTxt);
        CustomLayoutReporting.ProcessReportForData(
          ReportSelections.Usage::"C.Statement", RecRef, Customer.FieldName("No."), DATABASE::"Student Master-CS", Customer.FieldName("No."), true);
    end;
