codeunit 50002 "Bar Code-CS"
{
    ...
    var
        ...
        WhiteSpaces: Integer;
        // Standard128BENLbl: Label ' !"#$%&''()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~';
        // Standard128BDELbl: Label ' !"#$%&''()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ^_`abcdefghijklmnopqrstuvwxyzäöüß';
        Standard128BLbl: Label ' !"#$%&''()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ^_`abcdefghijklmnopqrstuvwxyzäöüß';
    ...
    begin
        ...
        FOR Line := 1 TO BMPLines DO BEGIN
            ...
            IF C128PlacePicRight AND (WhiteSpaces > 0) THEN
            ...
            FOR i := 1 TO BarcodeTextLen DO BEGIN // analyze barcodetext and generate bars
                CharCode := STRPOS(Standard128BLbl, FORMAT(BarcodeText[i])); // which number has the actual charater
            ...
            IF (NOT C128PlacePicRight) AND (WhiteSpaces > 0) THEN
                FOR i := 1 TO (WhiteSpaces * 3) DO
                    OStrm.WRITE(cWhite, 1);
    ...
    begin
        ...
        BMPSize := 54 + ROUND((Rows * 3), 4, '>') * Cols;
    ...
    procedure CreateBarcode(var No: Code[20]; var CompRec: Record "Company Information")
    begin
        CompRec.INIT();
        C128MakeBarcode(No, CompRec, 5300, 423, 96, TRUE);
    end;
}
