page 50682 "Waiver Journal"
{
    ...
    // // By default this page opens up in Simple mode;Rec.if users chooses to switch to classic mode (show more columns) then we remember their selection in Journal User Preferences table

    ApplicationArea = Basic, Suite;
    AutoSplitKey = true;
    Caption = 'Waiver Journals';
        ...
        area(content)
        {
            group(Control120)
            {
                ...
                field("<CurrentCurrencyCode>"; CurrentCurrencyCode)
                {
                    ...
                    trigger OnValidate()
                    begin
                        //GenJnlManagement.CheckCurrencyCode(CurrentCurrencyCode);//GMCSCOM
                    ...
                    begin
                        ...
                        if PAGE.RunModal(PAGE::"Standard General Journals", StdGenJnl) = ACTION::LookupOK then begin
                            if IsSimplePage then
                                ...
                                //StdGenJnl.CreateGenJnlFromStdJnlWithDocNo(StdGenJnl, CurrentJnlBatchName, CurrentDocNo)//GMCSCOM
                                //else
    ...
    begin
        ...
        // GenJnlManagement.TemplateSelection(PAGE::"Waiver Journal", 0, false, Rec, JnlSelected);//GMCSCOM
    ...
    begin
        if IsSimplePage then begin
            ...
            Rec.SetCurrentKey("Document No.", Rec."Line No.");
    ...
    begin
        ...
        if Rec.Count() = 0 then
            ...
            if ((xRec."Document No." = '') or (xRec."Journal Batch Name" <> Rec."Journal Batch Name")) and (not IsChangingDocNo) then
                ...
                Rec.Validate(Rec."Credit Amount", 0);
                Rec.Validate(Rec."Debit Amount", 0);
            end
        else
            Rec."Document No." := CurrentDocNo;
        ...
        if CurrentPostingDate <> 0D then
            Rec.Validate(Rec."Posting Date", CurrentPostingDate);
    end;
