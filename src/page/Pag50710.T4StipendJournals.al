page 50710 "T4 Stipend Journal"
{
    ...
    layout
    {
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                // field("Foreign Exchange Indicator"; "Foreign Exchange Indicator")//GMCSCOM
                // {
                //     ApplicationArea = Basic, Suite;
                ...
                //     Visible = false;
                // }
                // field("Foreign Exchange Ref.Indicator"; "Foreign Exchange Ref.Indicator")//GMCSCOM
                // {
                //     ApplicationArea = Basic, Suite;
                ...
                group("Electronic Payments")
                {
                    ...
                    action(ExportPaymentsToFile)
                    {
                        ...
                        begin
                            ...
                            begin
                                ...
                                //if BankAccount."Last Remittance Advice No." = '' then//GMCSCOM
                    ...
                    action(VoidPayments)
                    {
                        ...
                        begin
                            ...
                            //if not EntriesToVoid(GenJnlLine, false) then//GMCSCOM
                            // Error(NoEntriesToVoidErr);
                            // Clear(VoidTransmitElecPayments);
                            // VoidTransmitElecPayments.SetUsageType(1);
                            // // Void
    ...
    begin
        ...
        //GenJnlManagement.TemplateSelection(PAGE::"T4 Stipend Journal", 4, false, Rec, JnlSelected);//GMCSCOM
        ...
        if GenJournalBatch.Get(Rec."Journal Template Name", CurrentJnlBatchName) then
            if BankAccount.Get(GenJournalBatch."Bal. Account No.") then
                if GenJournalBatch."Bal. Account Type" = GenJournalBatch."Bal. Account Type"::"Bank Account" then
                    //if (BankAccount."Export Format" = 0) or (BankAccount."Export Format" = BankAccount.Rec."Export Format"::Other) then//GMCSCOM
                        AMCFormat := true;
    end;
