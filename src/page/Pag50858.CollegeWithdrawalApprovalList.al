Page 50858 "Pending College Withdrawal"
{

    PageType = List;
    SourceTable = "Withdrawal Approvals";
    SourceTableView = where("Type of Withdrawal" = filter("College-Withdrawal"), Status = FILTER("Pending for Approval"));
    Caption = 'Pending College Withdrawal Approvals List';
    ApplicationArea = All;
            ...
            action("&Approve")
            {
                ...
                begin
                    ...
                    IF CONFIRM(Text001Lbl, FALSE, Rec."Withdrawal No.") THEN BEGIN
                        ...
                        IF (Rec."Global Dimension 1 Code" = 'DEAN') Then
                            PendingCollegeWithdrawal;
    ...
    procedure PendingCollegeWithdrawal()
    var
    ...
    begin
        ...
        WithDepartment.SetRange("Department Code", '8006');
        ...
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('This is to inform you that the following student has requested a Withdrawal from' + Rec.Course + 'program,' + Rec.Semester + 'Semester due to <testing> effective from' + Format(Today));
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<br><br>');
        ...
        WebServicesFunctionsCod.ApiPortalinsertupdatesendNotification('Executive Dean', 'MEA', SenderAddress, Format(Rec."Department Name"),
        WithDepartment."Department Code", Subject, BodyText, 'Executive Dean', 'Executive Dean', '', '',
        Recipient, 1, '', '', 1);
    end;
    ...
    var
        ...
        Recipient: Text[100];
        DegreeName: text[2048];
        Withdrawalprocess: Record "Withdrawal Department";
    begin
        Withdrawalprocess.Reset();
        Withdrawalprocess.SetRange("Department Code", '8006');
        IF Withdrawalprocess.FindFirst() then;
        SmtpMailRec.Get();
        Recipient := Withdrawalprocess."User E-Mail";
        Recipients := Recipient.Split(';');
        SenderName := '';
        SenderAddress := SmtpMailRec."Email Address";
        Subject := (' ');
        ...
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('Dear' + '' + Withdrawalprocess."Department Code");
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('This is to inform you that the following student has requested a Withdrawal from' + Rec.Course + 'program' + Rec.Semester + 'Semester <CLN5>  due to <testing> effective from' + Format(Today));
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<br><br>');
        SmtpMail.AppendtoBody('<John Rublaitus>');
        SmtpMail.AppendtoBody('<2000295>');
        ...
        SmtpMail.AppendtoBody('<br>');
        // Mail_lCU.Send();GMCSCOM
        WebServicesFunctionsCod.ApiPortalinsertupdatesendNotification('Registrar', 'MEA', SenderAddress, Format(Rec."Department Name"),
     Withdrawalprocess."Department Code", Subject, BodyText, 'Registrar', 'Registrar', '', '',
    Recipient, 1, '', '', 1);
    end;
    ...
    var
        ...
        AppyrovalsDelegatedMsg: label 'The selected approval requests have been delegated.';
