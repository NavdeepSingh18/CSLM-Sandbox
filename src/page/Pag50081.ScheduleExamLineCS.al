            ...
            repeater(Group)
            {
                ...
                field("Exam No."; Rec."Exam No.")
                {
                    ...
                    var
                        ...
                        // ExtExamPubList: Page "External Exam Published List";
                        // IntExamPubList: Page "Internal Exam Published List";
                    begin
                        if Rec."Exam No." <> '' then begin
                            if Rec."Examiner Type" = Rec."Examiner Type"::External then begin
                                ExtExamHdr.Reset();
                                ExtExamHdr.SetRange("No.", Rec."Exam No.");
                                if ExtExamHdr.FindFirst() then
                                    if ExtExamHdr.Status = ExtExamHdr.Status::Published then begin
                                        // Clear(ExtExamPubList);
                                        // ExtExamPubList.SetTableView(ExtExamHdr);
                                        // ExtExamPubList.Run();
                                    end
                                    else
                                ...
                                if Rec."Examiner Type" = Rec."Examiner Type"::Internal then begin
                                    IntExamHdr.Reset();
                                    IntExamHdr.SetRange("No.", Rec."Exam No.");
                                    if IntExamHdr.FindFirst() then
                                        if IntExamHdr.Status = IntExamHdr.Status::Published then begin
                                            // Clear(IntExamPubList);
                                            // IntExamPubList.SetTableView(IntExamHdr);
                                            // IntExamPubList.Run();
                                        end
                                        else
