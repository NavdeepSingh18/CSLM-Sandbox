page 50898 "Student For SFP List"
{
    // version V.001-CS
    ...
    SourceTableView = SORTING("No.")
                      ORDER(Ascending) where("Type of FA Roster" = FILTER(SFP));
    ...
    layout
    {
        area(content)
        {
            repeater(Group1)
            {
                ...
                // field("Immigration Hold";Rec."Immigration Hold")//GMCSCOM
                // {
                //     ApplicationArea = All;
                // }
            }
    ...
    actions
    {
        area(Processing)
        {
                ...
                group("Registrar/Academics")
                {
                    ...
                    group("Registrar/Academics Details")
                    {
                        ...
                        action("Page Cust Ledger Entries")
                        {
                            ...
                            RunPageLink = "Customer No." = FIELD("Original Student No."), "Enrollment No." = FIELD("Enrollment No."), Semester = field(Semester), "Academic Year" = Field("Academic Year"), Term = Field(Term);
            ...
            action("Immigration Signof")
            {
                Caption = '&Immigration Signoff';
                Visible = ImmigrationHoldVisible;
                Image = SignUp;
                ApplicationArea = All;
                Promoted = true;
                ...
                begin
                    ...
                    IF HoldCount > 1 then begin
                        IF CONFIRM(Text001Lbl, FALSE) THEN BEGIN
                            IF StudentMasterRec.FindSet() then
                                repeat
                                //FinancialAidSignoff(StudentMasterRec);//GMCSCOM
                                until StudentMasterRec.Next() = 0;
                            Message(Text003Lbl);
                            CurrPage.Update();
                        end else
    ...
    begin
        ...
        //     (UserSetup."Department Approver" = UserSetup."Department Approver"::" ") then
        //         FinancialAIDHoldVisible := true;
        //     if (UserSetup."Department Approver" = UserSetup."Department Approver"::"Immigration Department") or
        //     (UserSetup."Department Approver" = UserSetup."Department Approver"::" ") then
        //         ImmigrationHoldVisible := true;
        // end;
