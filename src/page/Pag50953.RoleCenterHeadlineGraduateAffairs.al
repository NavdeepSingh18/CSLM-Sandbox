page 50953 RoleCenterHeadlineGraAffairs
{
    caption = 'Role Center Headline Graduate Affairs';
    ...
    layout
    {
        area(content)
        {
            ...
            field(Headline31; hdl3Txt)
            {
                ...
                begin
                    MSPE.reset();
                    MSPE.SetRange("Processing Status", MSPE."Processing Status"::Pending);
                    if MSPE.findset() then begin
                        Page.RunModal(Page::"Pending MSPE Application List", MSPE);//50807
                    end;
                end;
            }
            field(healdline41; hdl4Txt)
            {
                visible = true;
                DrillDown = true;
                trigger OnDrillDown()
                begin
                    MSPE.reset();
                    MSPE.SetRange("Processing Status", MSPE."Processing Status"::"Review Required");
                    if MSPE.findset() then begin
                        Page.RunModal(Page::"Review Required MSPE App List");//50917
                    end;
                end;
            }
            field(healdline51; hdl5Txt)
            {
                visible = true;
                DrillDown = true;
                trigger OnDrillDown()
                begin
                    MSPE.reset();
                    MSPE.SetRange("Processing Status", MSPE."Processing Status"::Completed);
                    if MSPE.findset() then begin
                        Page.RunModal(Page::"Completed MSPE App List");//50918
                    end;
                end;
            }
        }
    ...
    var
        ...
        CountPendgMSPEApps: integer;
        DateV: Text;
        Monthv: Text;
        YearV: text;
        CountReviewReq: Integer;
        CountCompleted: Integer;
    ...
    begin
        ...
        MSPE.SetRange("Processing Status", MSPE."Processing Status"::Pending);
        if MSPE.FindSet() then begin
            CountPendgMSPEApps := MSPE.Count();
            hdl3Txt := 'There are total ' + Headlinesmgnt.Emphasize(Format(CountPendgMSPEApps)) + ' Pending MSPE Applications.';
        End Else begin
            hdl3Txt := 'There are No Pending MSPE Applications.';
        end;
        //3 end
        //4 start
        MSPE.reset();
        MSPE.SetRange("Processing Status", MSPE."Processing Status"::"Review Required");
        if MSPE.FindSet() then begin
            CountReviewReq := MSPE.Count();
            hdl4Txt := 'There are total ' + Headlinesmgnt.Emphasize(Format(CountReviewReq)) + ' Review Required MSPE Applications.';
        End Else begin
            hdl4Txt := 'There are No Review Required MSPE Applications.';
        end;
        //4 end
        //5 start
        MSPE.reset();
        MSPE.SetRange("Processing Status", MSPE."Processing Status"::Completed);
        if MSPE.FindSet() then begin
            CountCompleted := MSPE.Count();
            hdl5Txt := 'There are total ' + Headlinesmgnt.Emphasize(Format(CountCompleted)) + ' Completed MSPE Applications.';
        End Else begin
            hdl5Txt := 'There are No Completed MSPE Applications.';
        end;
        //5 end
    end;
}